...
export default function MonthlyReports() {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-md bg-danger-50 px-4 py-3 text-sm text-danger-700">{error}</div>
      )}

      {history.length === 0 ? (
        <p className="text-sm text-slate-500">No monthly reports yet. Reports are generated when you view a specific month.</p>
      ) : (
        <>
          ...
          {!reportLoading && report && (
            <div className="space-y-6">
              ...
              {/* Category Breakdown */}
              <div>
                <h3 className="mb-3 text-base font-medium text-slate-700">Category Breakdown</h3>
                      ...
                      {report.categoryBreakdown.map((cat) => {
                        ...
                        return (
                          <tr key={cat.category} className="border-b border-slate-50 last:border-0">
                            <td className="px-4 py-3 text-slate-700">{cat.category}</td>
                            <td className="px-4 py-3 text-right text-slate-700">{formatCurrency(cat.limit)}</td>
                            <td className="px-4 py-3 text-right text-slate-700">{formatCurrency(cat.spent)}</td>
                            <td className={`px-4 py-3 text-right font-medium ${isOver ? 'text-danger-600' : 'text-success-600'}`}>
                              {formatCurrency(remaining)}
                            </td>
                            <td className="px-4 py-3 text-right">
                              {isOver ? (
                                <span className="rounded-full bg-danger-50 px-2 py-0.5 text-xs font-medium text-danger-600">Over</span>
                              ) : cat.spent === 0 ? (
                                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-400">Unused</span>
                              ) : (
                                <span className="rounded-full bg-success-50 px-2 py-0.5 text-xs font-medium text-success-600">OK</span>
                              )}
                            </td>
                          </tr>
                        );
              ...
              {/* History List */}
              <div>
                <h3 className="mb-3 text-base font-medium text-slate-700">All Months</h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {history.map((entry) => (
                    ...
                    >
                      <p className="font-medium text-slate-800">{getMonthName(entry.month)} {entry.year}</p>
                      <div className="mt-1 flex justify-between text-sm">
                        <span className="text-slate-500">Spent: {formatCurrency(entry.totalSpent)}</span>
